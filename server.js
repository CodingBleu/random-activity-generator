...
// Funktion zur Suche nach einer Aktivität basierend auf der Teilnehmeranzahl, Kategorie und Standort
function fetchActivityByCategory(
  participants,
  category,
  location,
  res,
  ...
  db.get(
    "SELECT description FROM activities WHERE participants = ? AND category = ? AND location = ? ORDER BY RANDOM() LIMIT 1",
    [participants, category, location],
    (err, row) => {
      if (err) {
...
app.get("/random-activity", (req, res) => {
  ...
  if (category === "random") {
    ...
    fetchActivityByCategory(participants, category, location, res, () => {
      res.status(404).send("Keine Aktivitäten gefunden");
    });
  }
});
  ...
  if (!cache[contentPath]) {
    ...
    const version =
      ...
      fileInfo.mtime
        .toISOString()
        .replace(/[^0-9]/g, "")
        .substring(0, 14); // Versionsstring wird erzeugt
    ...
    setTimeout(() => {
      delete cache[contentPath];
    }, 60000); // Cache für 1 Minute
